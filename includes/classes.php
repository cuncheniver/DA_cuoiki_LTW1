...
function menu($user) {
	...
	if($user !== false) {
		...
		$TMPL['avatar'] = permalink($CONF['url'].'/thumb.php?t=a&w=50&h=50&src='.$user['image']);
							...
							<div class="message-inner">
								...
								<div class="message-avatar" id="avatar'.$row['id'].'">
									<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage">
										<img src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$row['image']).'">
									</a>
								</div>
								<div class="message-top">
...
class manageUsers {
	...
	function getUser($id, $profile = null) {
		if($profile) {
			$query = sprintf("SELECT `idu`, `username`, `email`, `first_name`, `last_name`, `location`, `website`, `bio`, `facebook`, `twitter`, `gplus`, `born`, `verified` FROM `users` WHERE `username` = '%s' AND `suspended` != 2", $this->db->real_escape_string($profile));
		} else {
			$query = sprintf("SELECT `idu`, `username`, `email`, `first_name`, `last_name`, `location`, `website`, `bio`, `facebook`, `twitter`, `gplus`, `born`, `verified` FROM `users` WHERE `idu` = '%s' AND `suspended` != 2", $this->db->real_escape_string($id));
		}
		$result = $this->db->query($query);
...
class feed {
					...
					<div class="message-inner">
						...
						<div class="message-top">
							...
							<div class="message-author" id="author-p-'.$row['id'].'">
								<a href="'.$profile_url.'" rel="loadpage">'.$profile_name.'</a>'.$shared_title.$group_title.'
							</div>
							<div class="message-time">
	...
	function getGroup($start, $group, $from = null) {
		...
		if($this->group_data['name']) {
			...
			$query = sprintf("SELECT * FROM `messages` LEFT JOIN `users` ON `users`.`idu` = `messages`.`uid` AND `users`.`suspended` = 0 WHERE `messages`.`group` = '%s' AND `messages`.`public` = 1 %s %s ORDER BY `messages`.`id` DESC LIMIT %s", $group, $start, $from, ($this->per_page + 1));
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$group['cover'].'\', \''.($group['id'] ?? null).($group['title'] ?? null).'\', \'covers\', 0)" id="'.$group['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/thumb.php?t=c&w=900&h=200&src='.$group['cover']).')">
							</div></a>
		...
		if($type == 1) {
			...
			if($this->group_member_data['status'] == '1') {
				...
				if($this->group_data['privacy'] == 1) {
					...
					$this->db->query(sprintf("UPDATE `groups` SET `members` = `members` + 1 WHERE `id` = '%s'", $this->group_data['id']));
				}
			}
	...
	function sidebarGroupInfo($group) {
		...
		$info = '<div class="sidebar-container widget-group-info"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['profile_about'].''.(($this->group_member_data['permissions'] == 2) ? ' <span class="sidebar-header-link"><a href="'.permalink($this->url.'/index.php?a=group&name='.$group['name'].'&r=edit').'" rel="loadpage">'.$LNG['admin_ttl_edit'].'</a></span>' : '').'</div>';
	...
	function sidebarPageInfo($page) {
		...
		$info = '<div class="sidebar-container widget-group-info"><div class="sidebar-content"><div class="sidebar-header">'.$LNG['profile_about'].''.(($this->page_data['by'] == $this->id) ? ' <span class="sidebar-header-link"><a href="'.permalink($this->url.'/index.php?a=page&name='.$page['name'].'&r=edit').'" rel="loadpage">'.$LNG['admin_ttl_edit'].'</a></span>' : '').'</div>';
	...
	function fetchPage($page) {
		...
		$cover = '<div class="twelve columns">
					<div class="cover-container">
						<div class="cover-content">
							<a onclick="gallery(\''.$page['cover'].'\', \''.$page['id'].$page['name'].'\', \'covers\', 0)" id="'.$page['cover'].'"><div class="cover-image" style="background-position: center; background-image: url('.permalink($this->url.'/thumb.php?t=c&w=900&h=200&src='.$page['cover']).')">
							</div></a>
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$page['image'].'\', \''.$page['id'].$page['name'].'\', \'avatars\', 0)" id="'.$page['image'].'"><span id="avatar'.$page['id'].$page['name'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=150&h=150&src='.$page['image']).'"></span></a>
									</div>
								</div>
					...
					<div class="cover-container">
						<div class="cover-content">
							...
							<div class="cover-description">
								<div class="cover-avatar-content">
									<div class="cover-avatar">
										<a onclick="gallery(\''.$profile['image'].'\', \''.$profile['idu'].$profile['username'].'\', \'avatars\', 0)" id="'.$profile['image'].'"><span id="avatar'.$profile['idu'].$profile['username'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=150&h=150&src='.$profile['image']).'"></span></a>
									</div>
								</div>
	...
	function profileCard($data, $type) {
		...
		$card = '
			<div class="profile-card-cover">
				<img src="'.permalink($this->url.'/thumb.php?t=c&w=300&h=100&src='.$data['cover']).'">
				<div class="profile-card-info">
					<a href="'.$url.'" rel="loadpage"><div class="cover-username">'.$name.''.((!empty($data['verified'])) ? '<img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'" height="16" width="16">' : '').'</div></a>
				</div>
			</div>
			<div class="profile-card-avatar">
				<a href="'.$url.'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=112&h=112&src='.$data['image']).'"></a>
			</div>
		...
		$widget =  '<div class="sidebar-container widget-welcome">
						<div class="sidebar-content">
							...
							<div class="sidebar-inner">
								<div class="sidebar-avatar"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$username).'" rel="loadpage"><img src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$image).'"></a></div>
			...
			foreach($array as $value) {
				...
				if($value['event'] == 'like') {
					...
					if($for == 2 || !$for) {
						...
						$events .= '<div class="'.$notification_image.'"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$value['username']).'" rel="loadpage"><img class="notifications" src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$value['image']).'"></a></div><div class="notification-text notification-friendships"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$value['username']).'" rel="loadpage">'.realName($value['username'], $value['first_name'], $value['last_name']).'</a><br><span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div><div class="notification-buttons" id="notification-buttons'.$value['id'].'"><div class="notification-button button-normal"><a onclick="friend(\''.$value['idu'].'\', \'3\', \''.$value['id'].'\')">'.$LNG['decline'].'</a></div><div class="notification-button button-active"><a onclick="friend(\''.$value['idu'].'\', \'2\', \''.$value['id'].'\')">'.$LNG['confirm'].'</a></div></div>';
					}
		...
		if($result = $this->db->query($query)) {
			...
			foreach($rows as $comment) {
				...
				if($comment['type'] == 'picture') {
					$commentMedia = '<div class="comment-image-thumbnail"><a onclick="gallery(\''.$comment['value'].'\', '.$comment['id'].', \'media\', 2)" id="'.$comment['value'].'"><img src="'.permalink($this->url.'/thumb.php?t=m&w=200&h=200&src='.$comment['value']).'"></a></div>';
				}
				...
				<div class="message-reply-container" id="comment'.$comment['id'].'">
					...
					<div class="message-reply-message">
						<span class="message-reply-author" id="author-c-'.$comment['id'].'"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$comment['username']).'" rel="loadpage">'.$name.'</a></span>: <span id="comment_text'.$comment['id'].'">'.$this->parseMessage($comment['message']).'</span>
						'.$commentMedia.'
					</div>
	...
	function getChatType($type, $value, $id) {
		...
		switch($type) {
			case "picture":
			return '<div class="chat-image-thumbnail"><a onclick="gallery(\''.$value.'\', '.$id.', \'media\', 3)" id="'.$value.'"><img src="'.permalink($this->url.'/thumb.php?t=m&w=100&h=100&src='.$value).'"></a></div>';
		}
	}
	...
	function getMessageType($type, $value, $id) {
		...
		switch($type) {
			...
			// If it's a map
			case "map":
				return '<div class="message-type-map event-map"><img src="https://maps.googleapis.com/maps/api/staticmap?center='.$value.'&zoom=13&size=700x150&maptype=roadmap&markers=color:red%7C'.$value.'&sensor=false&scale=2&visual_refresh=true" referrerpolicy="no-referrer"></div>
				<div class="message-divider"></div>';
				break;
			...
			case "visited":
				return '<div class="message-type-general event-visited"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/visited.svg">'.sprintf($LNG['visited'], $value).'</div>
				<div class="message-divider"></div>';
				break;
			...
			case "music":
				...
				if(substr($value, 0, 3) == 'sc:') {
					...
					return '<div class="message-type-general event-music"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/music.svg">'.sprintf($LNG['listened'], $value).'</div>
					<div class="message-divider"></div>';
				}
				break;
			...
			case "video":
				if(substr($value, 0, 3) == 'yt:') {
					...
					return '<div class="message-type-general event-video"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/events/video.svg">'.sprintf($LNG['watched'], $value).'</div>
					<div class="message-divider"></div>';
				}
		...
		if(!empty($rows)) {
			if($for == 0) {
				...
				foreach($rows as $row) {
					...
					$output .= '<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage"><div class="sidebar-subscriptions"><div class="sidebar-title-container"><div class="sidebar-title-name">'.$username.'</div></div><img src="'.permalink($this->url.'/thumb.php?t=a&w=112&h=112&src='.$row['image']).'"></div></a>';
					$i++;
				}
				$output .= '</div></div></div>';
			} elseif($for == 1) {
		...
		if($type == 1) {
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$output .= '<div class="sidebar-users"><a href="'.permalink($this->url.'/index.php?a=messages&u='.$row['username'].'&id='.$row['idu']).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png" class="sidebar-status-icon"> <img src="'.permalink($this->url.'/thumb.php?t=a&w=25&h=25&src='.$row['image']).'"> '.realName($row['username'], $row['first_name'], $row['last_name']).'</a></div>';
					$output .= ($row['username'] == $_GET['u']) ? '</strong>' : '';
					$i++;
				}
			} else {
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					$url = ($window) ? '<a onclick="openChatWindow(\''.$row['idu'].'\', \''.$row['username'].'\', \''.addslashes(realName($row['username'], $row['first_name'], $row['last_name'])).'\', \''.$this->url.'\', \''.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png\')"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png" class="sidebar-status-icon"> <img src="'.permalink($this->url.'/thumb.php?t=a&w=25&h=25&src='.$row['image']).'"> '.realName($row['username'], $row['first_name'], $row['last_name']).'</a>' : '<a href="'.permalink($this->url.'/index.php?a=messages&u='.$row['username'].'&id='.$row['idu']).'" rel="loadpage"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/'.$icon.'.png" class="sidebar-status-icon"> <img src="'.permalink($this->url.'/thumb.php?t=a&w=25&h=25&src='.$row['image']).'"> '.realName($row['username'], $row['first_name'], $row['last_name']).'</a>';
			...
			if(!empty($rows)) {
				...
				foreach($rows as $row) {
					...
					<div class="message-reply-container '.$class.'" data-chat-id="'.$row['id'].'" >
						'.$delete.'
						<div class="message-reply-avatar">
							<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage" title="'.realName($row['username'], $row['first_name'], $row['last_name']).'" id="avatar-m-'.$row['id'].'"><img src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$row['image']).'"></a>
						</div>
	...
	function sidebarPages($visible = null) {
		...
		if($row) {
			...
			foreach($row as $page) {
				...
				$output .= '<div class="sidebar-link sidebar-page" id="page-'.$page['id'].'"'.$hidden.'><a href="'.permalink($this->url.'/index.php?a=page&name='.$page['name']).'" rel="loadpage"><img src="'.permalink($this->url.'/thumb.php?t=a&w=48&h=48&src='.$page['image']).'" width="24" height="24">'.($notifications ? '<span class="admin-notifications-number sidebar-notifications-number">'.$notifications.'</span>' : '').''.$page['title'].'</a><div class="sidebar-settings-container" onclick="messageMenu('.$page['id'].', 3)"><div class="settings_btn sidebar-settings'.($visible ? '' : ' s-settings-hidden').'"></div></div></div>';
								...
								<div class="message-inner">
									...
									<div class="message-top">
										<div class="message-author" id="author'.$row['idu'].'">
											<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage">'.$row['username'].'</a>'.((!empty($row['verified'])) ? '<span class="verified-small"><img src="'.$this->url.'/'.$CONF['theme_url'].'/images/icons/verified.png" title="'.$LNG['verified_page'].'"></span>' : '').'
										</div>
										<div class="message-time">
											'.realName(null, $row['first_name'], $row['last_name']).''.((!empty($row['location']) && $row['private'] == 0) ? ' ('.$row['location'].')' : '&nbsp;').' 
										</div>
	...
	function getMentions($target, $string) {
		...
		if($query->num_rows > 0) {
			...
			while($row = $query->fetch_assoc()) {
				$output .= '<div class="mention-user" onclick="doMention(\''.htmlspecialchars($target, ENT_QUOTES, 'UTF-8').'\', \''.htmlspecialchars($string, ENT_QUOTES, 'UTF-8').'\', \''.$row['username'].'\')"><img src="'.permalink($this->url.'/thumb.php?t=a&w=25&h=25&src='.$row['image']).'" width="16" height="16"> <strong>@'.$row['username'].'</strong> '.realName(null, $row['first_name'], $row['last_name']).'</div>';
			}
			$output .= '</div>';
		} else {
			return false;
	...
	function sidebarPlaces($id) {
		...
		if($this->id == $id) {
			$query = $this->db->query(sprintf("SELECT * FROM `messages` WHERE `messages`.`uid` = '%s' AND `messages`.`group` = 0 AND `messages`.`type` = 'map' ORDER BY `messages`.`id` DESC", $this->db->real_escape_string($id)));
		} else {
			$query = $this->db->query(sprintf("SELECT * FROM `messages` WHERE `messages`.`uid` = '%s' AND `messages`.`group` = 0 AND `messages`.`type` = 'map' AND `messages`.`public` = '1' ORDER BY `messages`.`id` DESC", $this->db->real_escape_string($id)));
		}
		...
		if(!empty($rows)) {
			$i = 0;
			$output = '<div class="sidebar-container widget-places"><div class="sidebar-content"><div class="sidebar-header"><a href="'.permalink($this->url.'/index.php?a=profile&u='.((!
			...
			foreach($rows as $row) {
				...
				$output .= '<a href="'.permalink($this->url.'/index.php?a=post&m='.$row['id']).'" rel="loadpage"><div class="sidebar-subscriptions"><div class="sidebar-title-container"><div class="sidebar-places-name">'.$row['value'].'</div></div><img src="https://maps.googleapis.com/maps/api/staticmap?center='.$row['value'].'&zoom=13&size=150x150&maptype=roadmap&sensor=false&scale=2&visual_refresh=true" referrerpolicy="no-referrer"></div></a>';
	...
	function sidebarFriendsActivity($limit, $type = null) {
		...
		/* $checkMessages = $this->db->query(sprintf("SELECT * FROM `messages` USE INDEX(`news_feed`) LEFT JOIN `users` ON `users`.`idu` = `messages`.`uid` AND `users`.`suspended` = 0 WHERE `messages`.`uid` IN (%s) AND `messages`.`public` = '1' AND `messages`.`page` = 0 AND `messages`.`group` = 0 ORDER BY `id` DESC LIMIT %s", $friendslist, 25));
		...
		foreach($array as $value) {
			...
			if($value['event'] == 'like') {
				$activity .= '<div class="sidebar-fa-image"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$value['username']).'" rel="loadpage"><img class="notifications" src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$value['image']).'"></a></div><div class="sidebar-fa-text">'.sprintf($LNG['new_like_fa'], permalink($this->url.'/index.php?a=profile&u='.$value['username']), realName($value['username'], $value['first_name'], $value['last_name']), permalink($this->url.'/index.php?a=post&m='.$value['post'])).'. <span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
			} elseif($value['event'] == 'comment') {
				$activity .= '<div class="sidebar-fa-image"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$value['username']).'" rel="loadpage"><img class="notifications" src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$value['image']).'"></a></div><div class="sidebar-fa-text">'.sprintf($LNG['new_comment_fa'], permalink($this->url.'/index.php?a=profile&u='.$value['username']), realName($value['username'], $value['first_name'], $value['last_name']), permalink($this->url.'/index.php?a=post&m='.$value['mid'])).'. <span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
			} elseif($value['event'] == 'message') {
				$activity .= '<div class="sidebar-fa-image"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$value['username']).'" rel="loadpage"><img class="notifications" src="'.permalink($this->url.'/thumb.php?t=a&w=50&h=50&src='.$value['image']).'"></a></div><div class="sidebar-fa-text">'.sprintf($LNG['new_message_fa'], permalink($this->url.'/index.php?a=profile&u='.$value['username']), realName($value['username'], $value['first_name'], $value['last_name']), permalink($this->url.'/index.php?a=post&m='.$value['id'])).'. <span class="timeago'.$b.'" title="'.$time.'">'.$time.'</span></div>';
			}
			$activity .= '</div></div>';
			$i++;
		}
	...
	function sidebarSuggestions($interests) {
		...
		$query = $this->db->query(sprintf("SELECT `idu`, `username`, `first_name`, `last_name`, `location`, `image`  FROM `users` WHERE `idu` NOT IN (%s) AND `gender` = '%s' AND `suspended` = 0 ORDER BY `idu` DESC LIMIT 6", $friendslist, $interests));
		...
		if(!empty($rows)) {
			...
			foreach($rows as $row) {
				...
				// Add the elemnts to the array
				$output .= '<a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" rel="loadpage"><div class="sidebar-subscriptions"><div class="sidebar-title-container"><div class="sidebar-title-name">'.realName($row['username'], $row['first_name'], $row['last_name']).'</div></div><img src="'.permalink($this->url.'/thumb.php?t=a&w=112&h=112&src='.$row['image']).'"></div></a>';
				$i++;
			}
			$output .= '</div></div></div>';
			return $output;
	...
	function getLikes($start = null, $type = null, $value = null, $for = null) {
		...
		if($type == 1) {
			...
			if($start) {
				...
				while($row = $query->fetch_assoc()) {
					$rows .= '<div class="likes-users"><a href="'.permalink($this->url.'/index.php?a=profile&u='.$row['username']).'" title="'.realName($row['username'], $row['first_name'], $row['last_name']).'" rel="loadpage"><img src="'.permalink($this->url.'/thumb.php?t=a&w=112&h=112&src='.$row['image']).'"></a></div>';
				}
				$rows .= '</div>';
				return $rows;
			} else {
	...
	function getGroups($start = null, $value = null, $live = null) {
		...
		if($live) {
					...
					$output .= '<div class="message-inner" id="group'.$row['id'].'">
									...
									<div class="message-avatar">
										<a href="'.permalink($this->url.'/index.php?a=group&name='.$row['name']).'" rel="loadpage">
											<img src="'.permalink($this->url.'/thumb.php?t=c&w=48&h=48&src='.$row['cover']).'">
										</a>
									</div>
									<div class="message-top">
										<div class="message-author">
				...
				$output .= '<div class="'.$class.'" id="group'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-avatar">
											<a href="'.permalink($this->url.'/index.php?a=group&name='.$row['name']).'" rel="loadpage">
												<img src="'.(isset($admin) ? $this->url.'/thumb.php?t=c&w=48&h=48&src='.$row['cover'] : permalink($this->url.'/thumb.php?t=c&w=48&h=48&src='.$row['cover'])).'">
											</a>
										</div>
										<div class="message-top">
											<div class="message-author">
			...
			if($loadmore) {
                $output .= '<div class="load_more" id="'.(empty($value) && empty($this->profile_data['idu']) ? 'more_users' : 'more_messages').'"><a onclick="group('.(empty($value) ? 5 : 3).', \''.htmlspecialchars($value).'\', \''.$this->db->real_escape_string($start + $this->per_page).'\', \''.$this->profile_data['idu'].'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>';
			}
		}
		return $output;
	}
	...
	function getPages($start = null, $value = null, $live = null) {
		...
		if($live) {
					...
					$output .= '<div class="message-inner" id="page'.$row['id'].'">
									...
									<div class="message-avatar">
										<a href="'.permalink($this->url.'/index.php?a=page&name='.$row['name']).'" rel="loadpage">
											<img src="'.permalink($this->url.'/thumb.php?t=a&w=48&h=48&src='.$row['image']).'">
										</a>
									</div>
									<div class="message-top">
										<div class="message-author">
				...
				$output .= '<div class="'.$class.'" id="page'.$row['id'].'">
								<div class="message-content">
									<div class="message-inner">
										...
										<div class="message-avatar">
											<a href="'.permalink($this->url.'/index.php?a=page&name='.$row['name']).'" rel="loadpage">
												<img src="'.(isset($admin) ? $this->url.'/thumb.php?t=a&w=48&h=48&src='.$row['image'] : permalink($this->url.'/thumb.php?t=a&w=48&h=48&src='.$row['image'])).'">
											</a>
										</div>
										<div class="message-top">
											<div class="message-author">
			...
			if($loadmore) {
                $output .= '<div class="load_more" id="'.((empty($value) && empty($this->profile_data['idu'])) ? 'more_users' : 'more_messages').'"><a onclick="page('.(empty($value) ? 2 : 1).', \''.htmlspecialchars($value).'\', \''.$this->db->real_escape_string($start + $this->per_page).'\', \''.$this->profile_data['idu'].'\')" id="load-more">'.$LNG['view_more_messages'].'</a></div>';
			}
		}
		return $output;
	}
	...
	function validateMessage($message, $image, $type, $value, $privacy, $group, $page) {
		...
		if($x == 1) {
			...
			$allowedType = array('map', 'game', 'video', 'food', 'visited', 'music', 'plugin');
						...
						if($type == 'video') {
							if(substr($value, 0, 20) == "https://youtube.com/" || substr($value, 0, 24) == "https://www.youtube.com/" || substr($value, 0, 16) == "www.youtube.com/" || substr($value, 0, 12) == "youtube.com/" || substr($value, 0, 19) == "http://youtube.com/" || substr($value, 0, 23) == "http://www.youtube.com/" || substr($value, 0, 16) == "http://youtu.be/" || substr($value, 0, 17) == "https://youtu.be/") {
								...
								if(substr($value, 0, 16) == 'http://youtu.be/' || substr($value, 0, 17) == "https://youtu.be/") {
									$value = htmlspecialchars(str_replace(array('http://youtu.be/', 'https://youtu.be'), 'yt:', $value), ENT_QUOTES, 'UTF-8');
								} else {
									$value = htmlspecialchars('yt:'.$my_array_of_vars['v'], ENT_QUOTES, 'UTF-8');
								}
								...
								$value = htmlspecialchars('vm:'.(int)substr(parse_url($value, PHP_URL_PATH), 1), ENT_QUOTES, 'UTF-8');
							}
						} elseif($type == 'music') {
							if(substr($value, 0, 23) == "https://soundcloud.com/" || substr($value, 0, 27) == "https://www.soundcloud.com/" || substr($value, 0, 22) == "http://soundcloud.com/" || substr($value, 0, 22) == "http://www.soundcloud.com/" || substr($value, 0, 15) == "soundcloud.com/" || substr($value, 0, 19) == "www.soundcloud.com/") {
								$value = htmlspecialchars('sc:'.parse_url($value, PHP_URL_PATH), ENT_QUOTES, 'UTF-8');
							}
						}
					}
				} else {
	...
	function groupMember($type, $user) {
		...
		if($type == 0 && $old['status'] == 1 && $new['status'] === NULL) {
			$remove = 1;
		}
...
function showUsers($users, $url) {
	...
	foreach($users as $user) {
		$x .= '<div class="welcome-user"><a href="'.permalink($url.'/index.php?a=profile&u='.$user['username']).'" rel="loadpage"><img src="'.permalink($url.'/thumb.php?t=a&w=112&h=112&src='.$user['image']).'"></a></div>';
	}
	return $x;
}
